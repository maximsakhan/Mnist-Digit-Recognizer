...
async function train(model, onIteration) {
  ...
  let valAcc;
  const gaugeContainer = document.getElementById('child3');
  await model.fit(trainData.xs, trainData.labels, {
    ...
    callbacks: {
      onBatchEnd: async (batch, logs) => {
        const accuracy = logs.acc * 100; // Get accuracy in percentage
...
// Function to update the needle position based on accuracy
function updateGauge(accuracy) {
  // Calculate the rotation angle based on accuracy
  const rotation = (accuracy / 100) * 180 - 90; // Assuming gauge has a range of 180 degrees
